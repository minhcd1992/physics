...
Trả lời bằng tiếng Việt, ngắn gọn (3-5 câu), thân thiện và tò mò như một AI đang học hỏi."""
                    ...
                    if has_api:
                        with st.spinner("AI đang suy nghĩ..."):
                            try:
                                # Sử dụng gemini-pro - phiên bản ổn định và phổ biến nhất
                                model = genai.GenerativeModel("gemini-pro")
                                
                                # Chuyển đổi lịch sử chat sang định dạng Gemini
                                gemini_history = []
                                for i, m in enumerate(st.session_state.ai_chat_history):
                                    role = "user" if m["role"] == "user" else "model"
                                    content = m["content"]
                                    
                                    # MẸO: Nhúng trực tiếp System Prompt vào tin nhắn đầu tiên 
                                    # để đảm bảo mọi phiên bản Gemini đều hiểu được luật chơi
                                    if i == 0 and role == "user":
                                        content = f"HƯỚNG DẪN DÀNH CHO AI:\n{system_prompt}\n\n---\nLỜI GIẢI THÍCH TỪ HỌC SINH:\n{content}"
